...
for(f=0;f<m.length;f++)if(m[f])h=true;f=false;if(m.length&&b.iframe!==false||b.iframe||h||f)b.closeKeepAlive?c.get(b.closeKeepAlive,l):l();else c.ajax(b);this.trigger("form-submit-notify",[this,b]);return this;function l(){var n=g[0];if(c(":input[name=submit]",n).length)alert('Error: Form elements must not be named "submit".');else{function q(y,w){if(window.ActiveXObject){w=new ActiveXObject("Microsoft.XMLDOM");w.async="false";w.loadXML(y)}else w=(new DOMParser).parseFromString(y,"text/xml");return w&&
w.documentElement&&w.documentElement.tagName!="parsererror"?w:null}function p(){if(!D++){x.detachEvent?x.detachEvent("onload",p):x.removeEventListener("load",p,false);var y=true;try{if(E)throw"timeout";var w,u;u=x.contentWindow?x.contentWindow.document:x.contentDocument?x.contentDocument:x.document;var A=o.dataType=="xml"||u.XMLDocument||c.isXMLDoc(u);d("isXml="+A);if(!A&&(u.body==null||u.body.innerHTML=="")){if(--I){D=0;setTimeout(p,100);return}d("Could not access iframe DOM after 50 tries.");return}t.responseText=
u.body?u.body.innerHTML:null;t.responseXML=u.XMLDocument?u.XMLDocument:u;t.getResponseHeader=function(J){var K={"content-type":o.dataType};return K[J]};if(o.dataType=="json"||o.dataType=="script"){var F=u.getElementsByTagName("textarea")[0];if(F)t.responseText=F.value;else{var G=u.getElementsByTagName("pre")[0];if(G)t.responseText=G.innerHTML}}else if(o.dataType=="xml"&&!t.responseXML&&t.responseText!=null)t.responseXML=q(t.responseText);w=c.httpData(t,o.dataType)}catch(L){y=false;c.handleError(o,
t,"error",L)}if(y){o.success(w,"success");B&&c.event.trigger("ajaxSuccess",[t,o])}B&&c.event.trigger("ajaxComplete",[t,o]);B&&!--c.active&&c.event.trigger("ajaxStop");if(o.complete)o.complete(t,y?"success":"error");setTimeout(function(){z.remove();t.responseXML=null},100)}}var o=c.extend({},c.ajaxSettings,b),v=c.extend(true,{},c.extend(true,{},c.ajaxSettings),o),C="jqFormIO"+(new Date).getTime(),z=c('<iframe id="'+C+'" name="'+C+'" src="'+o.iframeSrc+'" />'),x=z[0];z.css({position:"absolute",top:"-1000px",
left:"-1000px"});var t={aborted:0,responseText:null,responseXML:null,status:0,statusText:"n/a",getAllResponseHeaders:function(){},getResponseHeader:function(){},setRequestHeader:function(){},abort:function(){this.aborted=1;z.attr("src",o.iframeSrc)}},B=o.global;B&&!c.active++&&c.event.trigger("ajaxStart");B&&c.event.trigger("ajaxSend",[t,o]);if(v.beforeSend&&v.beforeSend(t,v)===false)v.global&&c.active--;else if(!t.aborted){var D=0,E=0;if(v=n.clk){var H=v.name;if(H&&!v.disabled){b.extraData=b.extraData||
{};b.extraData[H]=v.value;if(v.type=="image"){b.extraData[name+".x"]=n.clk_x;b.extraData[name+".y"]=n.clk_y}}}setTimeout(function(){var y=g.attr("target"),w=g.attr("action");n.setAttribute("target",C);n.getAttribute("method")!="POST"&&n.setAttribute("method","POST");n.getAttribute("action")!=o.url&&n.setAttribute("action",o.url);b.skipEncodingOverride||g.attr({encoding:"multipart/form-data",enctype:"multipart/form-data"});o.timeout&&setTimeout(function(){E=true;p()},o.timeout);var u=[];try{if(b.extraData)for(var A in b.extraData)u.push(c('<input type="hidden" name="'+
A+'" value="'+b.extraData[A]+'" />').appendTo(n)[0]);z.appendTo("body");x.attachEvent?x.attachEvent("onload",p):x.addEventListener("load",p,false);n.submit()}finally{n.setAttribute("action",w);y?n.setAttribute("target",y):g.removeAttr("target");c(u).remove()}},10);var I=50}}}};c.fn.ajaxForm=function(b){return this.ajaxFormUnbind().bind("submit.form-plugin",function(){c(this).ajaxSubmit(b);return false}).bind("click.form-plugin",function(f){var k=f.target,m=c(k);if(!m.is(":submit,input:image")){k=
...
var jsconf=function(){var c="home",d="<div class='article'><h3><a href='{{link}}'>{{title}}</a></h3><div class='dateline'>{{date}}</div><div class='body'>{{{body}}}</div>",b="<a href='{{link}}' class='video'><div class='img'><img height='90' width='120' src='{{flv}}.jpg'/></div><div class='title'>{{title}}</div></a>",f=870;function k(){return jQuery("#navigation").offset().left}function m(g){var e=0,j=0;if(g=="articles"||g=="schedule"||g=="videos")j=110;else if(g=="register"||g=="home"||g=="sponsors")j=
-439;else if(g=="about"||g=="speakers"||g=="venue")j=-990;var l=k();if(g=="articles"||g=="register"||g=="about")e=l+10;else if(g=="schedule"||g=="home"||g=="speakers")e=l-f;else if(g=="videos"||g=="sponsors"||g=="venue")e=l-(f*2+13);return g={top:j,left:e}}function h(g){var e=m(c);g?jQuery("#viewpane").animate(e):jQuery("#viewpane").css(e)}return{resizeContainer:function(){h()},focus_on:function(g){c=g;h(true)},default_focus:function(){h(true)},load_blog:function(){var g=new google.feeds.Feed("http://feeds.feedburner.com/posterous/tzwp");
g.setNumEntries(10);g.load(function(e){if(!e.error){e.feed.entries.forEach(function(j,l){jQuery("#article_listing").append(Mustache.to_html(d,{title:j.title,body:j.content,date:Date.parse(j.publishedDate).toString("dddd, MMMM d, yyyy"),link:j.link}));l==0&&jQuery("#top_article").append(Mustache.to_html(d,{title:j.title,body:j.contentSnippet.replace("Permalink",""),date:Date.parse(j.publishedDate).toString("dddd, MMMM d, yyyy"),link:j.link}))});jQuery("#article_listing .article p:last").remove()}})},
load_videos:function(){var g=new google.feeds.Feed("http://blip.tv/?search=jsconf;s=search&skin=rss");g.setNumEntries(20);g.load(function(e){e.error||e.feed.entries.forEach(function(j){var l=j.title.split(" ").slice(0,2).join(" ").replace(/[^a-zA-z ]/,"");jQuery("#video_listing").append(Mustache.to_html(b,{title:l,link:j.link,flv:j.mediaGroups[0].contents[1].url}))})})},load_tweets:function(){jQuery("#twitter").tweet({avatar_size:28,count:3,query:"jsconf",loading_text:"searching twitter..."})},wrap_form:function(){$("#regform").ajaxForm({complete:function(g,
e){r=g;s=e;$("#submission").val(g.responseText)}})}}}();function initialize(){jsconf.resizeContainer();jQuery(window).resize(jsconf.resizeContainer);jQuery("div.block").click(function(){jsconf.focus_on(jQuery(this).attr("id"))});jQuery("#navigation a, a.focus").click(function(c){c.preventDefault();c.stopPropagation();jsconf.focus_on(jQuery(this).attr("href").replace("#",""))});jsconf.load_blog();jsconf.load_videos();jsconf.load_tweets();jsconf.wrap_form()}google.setOnLoadCallback(initialize);
